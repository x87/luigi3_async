...
import { flag_luigi_mission3_passed, flag_player_on_luigi_mission, misty_door1 } from "./scm_variables.mjs";
import { fading } from "./common.mjs";
import { asyncWait } from "./mission_runner";
...
async function body() {
  ...
  Cutscene.Start();
  await print("LM3_A", 2433); // "Hey I've gotta talk to you... All right Mick I talk to yah later"
  await waitCutscene(5504);
  Text.ClearThisPrint("LM3_A");
  await print("LM3_B", 8333); //"How yah doing kid?
  await print("LM3_C", 9667); //"The Don's son, Joey Leone, he wants some action from his regular girl Misty"
  await print("LM3_D", 13833); //"Go pick her up at Hepburn Heights..."
  await print("LM3_E", 15467); //"but watch yourself that's Diablo turf."
  await print("LM3_F", 18233); //"Then run her over to his garage in Trenton and make it quick,"
  await print("LM3_G", 21100); //"Joey ain't the kinda you keep waiting, remember, this is your foot in the door..."
  await print("LM3_H", 25333); //"so keep your eyes on the road and off Misty!"
  await waitCutscene(27701);
  Text.ClearThisPrint("LM3_H");
  await waitCutscene(29666);
  ...
  await waitCutscene(25723);
  Text.ClearThisPrint("LM3_9B");
  await waitCutscene(26666);
  Camera.DoFade(1500, Fade.Out);
  while (!Cutscene.HasFinished()) {
    await asyncWait(0);
  }
...
async function print(text, start) {
  await waitCutscene(start);
  Text.PrintNow(text, 10000, 1);
}

async function waitCutscene(timestamp) {
...
export default () => body().then(onPassed).catch(onFailed);
